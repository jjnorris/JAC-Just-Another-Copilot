...
class VerifyValidationReportStageSetTests(unittest.TestCase):
    def test_missing_required_validated_stages_fails_verification(self) -> None:
        ...
        try:
            ...
            self.assertIn("INVALID VALIDATED STAGES", result.stdout)
            self.assertIn("repo_task_edit_sketch.py", result.stdout)
            self.assertIn("profile_repo_stack.py", result.stdout)
        finally:
