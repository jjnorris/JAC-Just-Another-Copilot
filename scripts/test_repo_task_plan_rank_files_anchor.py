...
from scripts.repo_task_plan import rank_files, choose_recommended_first_edit_area, synthesize_implementation_steps, load_json
...
class TestRankFilesAnchor(unittest.TestCase):
    ...
    def tearDown(self):
        for p in (self.jack_dir / "repo-stack-profile.json", self.jack_dir / "repo-task-brief.json"):
            if p.exists():
                p.unlink()
    ...
    def test_planning_variant_anchors_rank_files(self):
        ...
        self.assertTrue(ranked[0].endswith("repo_task_plan.py"), "Planner should be ranked first when inspector reports def rank_files")
        ...
        steps = synthesize_implementation_steps(task, profile, brief, ranked[:5], recommended)
        self.assertTrue(any("rank_files" in s or "def rank_files" in s for s in steps), "Implementation steps should reference def rank_files for planner-focused tasks")
