...
class TestFreshnessEvidenceVerifier(unittest.TestCase):
    def test_freshness_evidence_verifier_passes(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            # Artifact family manifest (include freshness entry)
            entries = []
            ...
            # Add freshness evidence entry
            entries.append({
                "stage_name": "post_validation",
                "artifact_role": "freshness_evidence",
                "artifact_path": "jack/repo-task-freshness-evidence.json",
            ...
            # Freshness evidence artifact (POSIX-normalized paths)
            freshness = {
                "generated_at": now,
                "task": task,
                "source_inputs": [
                    (jack / "repo-docs-lookup-plan.json").relative_to(repo_root).as_posix(),
                ],
                "evidence_sources": [
                    (jack / "repo-docs-evidence.jsonl").relative_to(repo_root).as_posix(),
                ],
                "consumed_by_stages": [
                    "profile_to_docs_lookup.py",
                ],
            }
            (jack / "repo-task-freshness-evidence.json").write_text(json.dumps(freshness, indent=2), encoding="utf-8")

            # Run verifier with freshness gate enabled
