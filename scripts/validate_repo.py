...
def resolve_local_path(source: Path, ref: str) -> Path:
    ...
    docs_jac_candidate = (ROOT / "docs/jac" / candidate).resolve()
    if docs_jac_candidate.exists():
        return docs_jac_candidate
    return repo_candidate
...
def check_inventory(inventory: Any, failures: list[str]) -> None:
    if not isinstance(inventory, dict):
        failures.append("docs/jac/inventory.json: did not parse into a JSON object")
        return

    anchor_files = inventory.get("anchor_files", {})
    if not isinstance(anchor_files, dict):
        failures.append("docs/jac/inventory.json: anchor_files must be an object")
    else:
        for name, value in sorted(anchor_files.items()):
            if not isinstance(value, str):
                failures.append(f"docs/jac/inventory.json: anchor_files.{name} must be a string")
                continue
            if not (ROOT / value).exists():
                failures.append(f"docs/jac/inventory.json: anchor file missing: {value}")

    patterns = inventory.get("patterns", {})
    if not isinstance(patterns, dict):
        failures.append("docs/jac/inventory.json: patterns must be an object")
    else:
        for name, values in sorted(patterns.items()):
            if not isinstance(values, list):
                failures.append(f"docs/jac/inventory.json: patterns.{name} must be an array")
                continue
            for pattern in values:
                if not isinstance(pattern, str):
                    failures.append(f"docs/jac/inventory.json: patterns.{name} contains a non-string entry")
                    continue
                matches = [path for path in ROOT.glob(pattern) if path.exists()]
                if not matches:
                    failures.append(f"docs/jac/inventory.json: pattern matched no files: {pattern}")
...
def check_path_references(inventory: Any, failures: list[str]) -> None:
    ...
    if not isinstance(validation_targets, dict):
        failures.append("docs/jac/inventory.json: validation_targets must be an object")
        return
    ...
    if not isinstance(doc_targets, list):
        failures.append("docs/jac/inventory.json: validation_targets.path_reference_docs must be an array")
        doc_targets = []
    if not isinstance(json_patterns, list):
        failures.append("docs/jac/inventory.json: validation_targets.path_reference_json_patterns must be an array")
        json_patterns = []

    targets: list[Path] = []
    for doc in doc_targets:
        if not isinstance(doc, str):
            failures.append("docs/jac/inventory.json: validation target doc entries must be strings")
            continue
        path = ROOT / doc
        if not path.exists():
            failures.append(f"docs/jac/inventory.json: validation target missing: {doc}")
            continue
        targets.append(path)

    for pattern in json_patterns:
        if not isinstance(pattern, str):
            failures.append("docs/jac/inventory.json: validation target JSON patterns must be strings")
            continue
        matched = list(ROOT.glob(pattern))
        if not matched:
            failures.append(f"docs/jac/inventory.json: validation target JSON pattern matched no files: {pattern}")
            continue
...
def main() -> int:
    ...
    if failures:
        print("JAC repository validation failed:")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("JAC repository validation passed.")
    return 0
