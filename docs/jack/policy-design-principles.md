...
## Reference material in `docs/jack/`

`docs/jack/` keeps the longer rules, workflows, templates, event contracts, examples, hook notes, and support docs.
...
1. **One clear home per policy.** If a rule already lives under `docs/jack/rules/`, avoid rewriting it elsewhere.
2. **Keep repo-scoped files concise.** Longer explanation belongs in `docs/jack/`.
...
6. **Keep reference material honest.** `docs/jack/` supports maintenance and manual fallback; it is not a claimed runtime surface.
