...
This document explains where JAC material belongs and how to keep the repository readable.
...
## Reference material in `docs/jac/`

`docs/jac/` keeps the longer rules, workflows, templates, event contracts, examples, hook notes, and support docs.
...
1. **One clear home per policy.** If a rule already lives under `docs/jac/rules/`, avoid rewriting it elsewhere.
2. **Keep repo-scoped files concise.** Longer explanation belongs in `docs/jac/`.
...
6. **Keep reference material honest.** `docs/jac/` supports maintenance and manual fallback; it is not a claimed runtime surface.
